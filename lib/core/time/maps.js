...
function filterVectorLayer(layer, range) {
    ...
    var features = [];
    for (var i=0, ii=layer._features.length; i<ii; ++i) {
        var feature = layer._features[i];
        var featureTime = Date.parse(feature.get(layer.get('timeAttribute')));
