(function() {
    ...
    StoryPinLayerManager.prototype.pinsChanged = function(pins, action, dontSave) {
        ...
        var range = storytools.core.utils.computeRange(this.storyPins, function(p) {
            return {
                start: p.start_time,
                end: p.end_time
            };
        });
        this.storyPinsLayer.set('times', range);
    ...
    module.service('stAnnotationsStore', function(mapid, StoryPin) {
        function path() {
        ...
        return {
            loadAnnotations: function(projection) {
                return StoryPin.createStoryPins(get(), projection);
            },
