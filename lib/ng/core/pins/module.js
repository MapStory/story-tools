(function() {
    ...
    function readGeometry(data, projection) {
        ...
        if (angular.isString(geom) || 'type' in geom) {
    ...
    function StoryPin(data, projection) {
        ...
        this.id = null;
        if (data) {
            this.update(data, projection);
        }
    }
    StoryPin.prototype.update = function(data, projection) {
        ...
        if (typeof data.the_geom != 'undefined') {
            ...
            if (this.the_geom) {
                ...
                this.$feature = createFeature(this, projection);
            } else {
                this.$feature = null;
            }
        } else if (this.the_geom) {
            // @todo we're always creating a new feature
            this.$feature = createFeature(this, projection);
        }
    };
    StoryPin.createStoryPins = function(data, projection) {
        ...
        return data.map(function(s) {
            return new StoryPin(s, projection);
        });
    };
    ...
    function StoryPinLayerManager(storyPins) {
        ...
        this.storyPins = [];
        this.pinsChanged(storyPins || [], 'add');
    }
    ...
    StoryPinLayerManager.prototype.loadFromGeoJSON = function(geojson, projection) {
        ...
        var pins = StoryPin.createStoryPins(annotations, projection);
        this.pinsChanged(pins, 'add', true);
    };
