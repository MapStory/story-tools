(function() {
  ...
  module.provider('mediaService', function() {
    ...
    function noembed_handler(url, embed_params) {
      ...
      fetch(request_url, {headers: {'Content-Type':'application/json'}}).
        then(rawResponse => rawResponse.text()).
        then((response) => {
          console.log('sdklsfjlksfdjkljkslfdkjlsfdjklsfdkjlsfdjklsfdjklfsdks', response);
      });
      ...
      return http_.jsonp(sce_.trustAsResourceUrl(request_url), {
        ...
        /*success*/
        function(result) {
          return Promise.resolve(result.data.html);
        },
        /*failure*/
        function(result) {
