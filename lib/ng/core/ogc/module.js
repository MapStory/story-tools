(function() {
  ...
  StoryLayer.prototype.setWMSSource = function() {
    ...
    var params = $.extend(this.get("params"), prms) || prms;
    if (times) {
  ...
  module.service("stAnnotateLayer", function($rootScope) {
    return {
      loadCapabilities: function(storyLayer, map) {
        console.log('LOADING capabilityes /**/*/**//*/*/*/*/**/*//**/*/');
        ...
        return fetch(`${url}?REQUEST=${request}&SERVICE=${service}&VERSION=1.3.0&TILED=true`)
          .then(rawResponse => {
            ...
            console.log('HTTP Rewrite', rawResponse);
          ...
          .catch(function(response) {
            console.log(url, 'failed to load', response);
          });
      },
      describeFeatureType: function(storyLayer) {
        ...
        return fetch(url)
          .then(rawResponse => {
            ...
            console.log("HTTTTTTTP response", rawResponse);
            return rawResponse.json().then(response => {
              ...
              if (parser) {
                var layerInfo = parser.parseResult(response);
      ...
      getTimeAttribute: function(storyLayer) {
        return fetch(storyLayer.get("timeEndpoint"))
          .then(rawResponse => {
            if (!rawResponse.ok) {
              return;
            }
            //TODO: return early if ok is false
            console.log('HTTTP 0-0-0-0--0-0', rawResponse);
            return rawResponse.json().then(response => {
    ...
    return {
      buildEditableLayer: function(data, map) {
        ...
        return Promise.all(promises).then(
          function() {
            console.log('PROMISES ALLLfljksdfdkljdfsjklsdfjlkfsdfjsdkldfsjlksjlfkdkl');
