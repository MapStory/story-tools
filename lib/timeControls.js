...
function TimeController(model, slider, timeline, controls) {
    ...
    function getTimelineWindow() {
        return utils.createRange(timeline._timeline.getWindow());
    }
    ...
    function centerTimeline(range) {
        ...
        timeline._timeline.moveTo(c, {animate: false});
        timeline._timeline.setCurrentTime(c);
    ...
    function adjustSlider(range) {
        ...
        timeline._timeline.setCurrentTime(center);
    ...
    timeline._timeline.on('rangechanged', function(range) {
    ...
    function move(amt) {
        ...
        if (started) {
            timeout = window.setTimeout(move, 1000, amt);
        }
    }
...
function MapController(map, model) {

}

function BoxController(map, model) {
...
function Box(options) {
    ...
    this.center = options.center; // ol.Coordinate
    this.resolution = options.resolution;
    this.fixed = options.static; // don't allow pan/zoom
 ...
 *      } ... ],
 *   data : [ date|range ...] | { range, interval },
 *   playback : {
...
function create(options) {
    ...
    var model = models.TimeModelRanged,
    ...
    if (Array.isArray(options.data)) {
        model = models.TimeModelInstant;
        totalRange = utils.computeRange(options.data);
        dataOrInterval = options.data;
    } else {
        totalRange = options.data;
        dataOrInterval = options.interval;
    }
    ...
    model = _create(model, playback.mode, playback.fixed, boxes, annotations, totalRange, dataOrInterval);
    boxes = new BoxController(options.map, model);
    ...
    return new TimeController(model, slider, timeline, controls);
}

function _create(clazz) {
    var x = Object.create(clazz);
    clazz.apply(x, Array.prototype.slice.call(arguments, 1));
    return x;
}
