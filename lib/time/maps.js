...
function TileLoadListener(options) {
    var tileStatusCallback = options.callback;
    var tilesLoading = {};
...
exports.MapController = function(options, timeControls) {
    ...
    function switchLayers() {
        map.getLayers().forEach(function(lyr) {
            if (lyr instanceof ol.layer.Group) {
                var bottom = lyr.getLayers().item(0);
                lyr.getLayers().removeAt(0);
                lyr.getLayers().push(bottom);
                lyr.getLayers().item(0).setVisible(false);
            }
        });
    }
    ...
    function createLoadListener(cancelCallback) {
        if (loadListener !== null) {
            loadListener.cancel();
        }
        loadListener = new TileLoadListener({cancelCallback: cancelCallback, callback: tileStatusCallback});
        return loadListener;
    }
    function updateLayers(range) {
        ...
        for (var i = 1; i < layers.getLength(); i++) {
            if (layers.item(i) instanceof ol.layer.Group) {
                var bottom = layers.item(i).getLayers().item(0);
                bottom.setVisible(true);
                bottom.getSource().updateParams({TIME: time});
            }
        }
        if (layers.getLength() > 1) {
            timeControls.defer(createLoadListener(switchLayers).deferred);
        }
    }
