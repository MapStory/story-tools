                ...
                if (layer.type === 'WMS') {
                    ...
                    if (layer.singleTile === true) {
                        ...
                        if (layer.resolutions && layer.origin) {
                            // calculate widths
                            var widths = [];
                            var xmin = layer.origin[0];
                            var ymax = layer.origin[1];
                            var ymin = layer.bbox[3];
                            var xmax = layer.bbox[2];
                            for (var r=0, rr=layer.resolutions.length; r<rr; ++r) {
                                var resolution = layer.resolutions[r];
                                var width = resolution * 256;
                                var numTilesWide = Math.ceil((xmax - xmin) / width);
                                widths.push(numTilesWide);
                            }
                            tileGrid = new ol.tilegrid.TileGrid({
